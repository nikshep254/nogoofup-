  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12 animate-fade-in">
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-slate-100 overflow-hidden mb-8">
        ...
        <div className="p-8">
            <h3 className="font-bold text-lg text-slate-800 mb-4">Best Performance</h3>
            {profile.bestMarks ? (
                <div className="grid grid-cols-4 gap-4 max-w-lg">
                    ...
                    <div className="bg-slate-50 p-3 rounded-lg text-center border-t-2 border-blue-400">
                        <div className="text-lg font-bold text-slate-700">{profile.bestMarks.physics}</div>
                        <div className="text-xs text-slate-500">Physics</div>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-lg text-center border-t-2 border-green-400">
                        <div className="text-lg font-bold text-slate-700">{profile.bestMarks.chemistry}</div>
                        <div className="text-xs text-slate-500">Chem</div>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-lg text-center border-t-2 border-red-400">
                        <div className="text-lg font-bold text-slate-700">{profile.bestMarks.maths}</div>
      ...
      <h3 className="font-bold text-xl text-slate-800 mb-4 px-2">Prediction History</h3>
      <div className="space-y-4">
        {profile.history.length > 0 ? (
            profile.history.map((item) => (
                <div key={item.id} className="bg-white p-5 rounded-xl shadow-sm border border-slate-100 flex items-center justify-between hover:border-brand-200 transition-colors">
                    <div>
                        ...
                        <div className="text-sm text-slate-500 mt-1">
                            Predicted Rank: <span className="text-brand-600 font-semibold">{item.metrics.rank.toLocaleString()}</span> • Percentile: {item.metrics.percentile.toFixed(2)}
                        </div>
                    </div>
                    <button 
